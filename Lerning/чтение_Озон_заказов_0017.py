...
def process_excel_by_cluster(input_file, output_file, column_name=None):
    """
    Создает файл Excel с листами по кластерам

    :param input_file: Путь к исходному файлу данных
    :param output_file: Путь для сохранения результата
    :param column_name: Название для столбца с данными (если None - будет использована текущая дата)
    """
    ...
    # Устанавливаем имя столбца
    if column_name is None:
        column_name = datetime.now().strftime("%d.%m.%Y")
    ...
    with pd.ExcelWriter(output_file, engine='openpyxl') as writer:
        ...
        for cluster in existing_clusters:
            ...
            # Переименовываем столбец с данными
            result_df = result_df.rename(columns={'Доступно к продаже': column_name})

            # Сортировка по убыванию
            result_df = result_df.sort_values(by=column_name, ascending=False)

            # Упорядочиваем столбцы
            result_df = result_df[['SKU', 'Артикул', 'Название товара', column_name]]
...
# Пример использования с ручным указанием имени столбца
process_excel_by_cluster(input_filename, output_filename,"18.06.2025")  # Можно указать любое имя или None для даты