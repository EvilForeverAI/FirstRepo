...
def process_excel_by_cluster(input_file, output_file):
    ...
    with pd.ExcelWriter(output_file, engine='openpyxl') as writer:
        for cluster in df['Кластер'].unique():
            ...
            # Сортировка по убыванию 'Доступно к продаже'
            result_df = result_df.sort_values(by='Доступно к продаже', ascending=False)

            # Упорядочиваем столбцы
            result_df = result_df[['SKU', 'Артикул', 'Название товара', 'Доступно к продаже']]
